...
def suppress_inference(stack: Stack, image_resource_wildcards: list[str]) -> None:
    ...
    _suppress_by_path(
        ...
        [
            ...
            NagPackSuppression(
                id="AwsSolutions-IAM5",
                reason=f"{ACCEPTED}: the object wildcard on each image bucket is "
                "CDK-generated by grant_read/grant_put and confined to that env's "
                "image key prefix (dev mermaid/, prod mermaid/ and "
                "mermaid-production-test/) — the Lambda reads every patch image under "
                "the prefix and writes each feature vector back beside it.",
...
def apply_all(
    ...
    # Each env's image-bucket grants resolve to different resource ARNs: dev reads the
    # in-account image-processing bucket, prod also reads coral-reef-training.
    suppress_inference(
        dev_inference_stack,
