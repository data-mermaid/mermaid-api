...
if headers:
    ...
    if project:
        ...
        for obs in belt_fish_observations['results']:
            site = get_site_from_obs(obs, project_beltfishs, project_beltfish_transects, sample_events, project_sites)
            biomass_per_site[site['name']] += get_biomass_kg_ha(obs, fish_attributes, project_beltfishs,
                                                                project_beltfish_transects, widths)
