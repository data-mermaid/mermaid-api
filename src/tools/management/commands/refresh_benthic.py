...
from api.models import BenthicAttribute, Region, BenthicLifeHistory, BaseAttributeModel, APPROVAL_STATUSES
...
class Command(BaseCommand):
    ...
    def __init__(self):
        ...
        self.source = os.path.join(settings.BASE_DIR, 'data', 'MVP_benthic_list_for_MERMAID_23July2018.csv')
        self.create_only = False

    def add_arguments(self, parser):
        parser.add_argument('-c', action='store_true', dest='create_only',
                            default=False, help='Only process create operations, not updates'),
        ...
        if name != '':
            ...
            try:
                ...
                if parent != attrib.parent and not self.create_only:
                    print('name: %s existing parent: %s new parent: %s' % (name, attrib.parent, parent))
    ...
    def handle(self, *args, **options):
        ...
        with open(self.source) as benthicdata:
            ...
            for row in csvreader:
                ...
                attrib2, c2 = self.benthic_attribute(col2, attrib1, col3, life_history, chosen_regions)
                attrib3, c3 = self.benthic_attribute(col3, attrib2, col4, life_history, chosen_regions)
                attrib4, c4 = self.benthic_attribute(col4, attrib3, '', life_history, chosen_regions)
                n += sum([c1, c2, c3, c4])
