...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-f', action='store_true', dest='force',
                            default=False, help='Force restore'),
        parser.add_argument('-n', action='store_true', dest='no_download', default=False, help='Do not download dumped '
                                                                                               'data from S3')
        ...
        if options.get('no_download', False) is True:
            ...
            for f in os.listdir(tmpdir):
                ...
                if os.path.isfile(localfile) and self.restore in localfile:
                    if download_file_name is None or os.path.getmtime(localfile) > os.path.getmtime(download_file_name):
            ...
            if bucket_file_list and len(bucket_file_list) > 0:
                ...
                for s3_obj in bucket_file_list:
                    if self.restore in s3_obj['Key']:
                        ...
                        elif s3_obj.get('LastModified') > latest_key_name.get('LastModified'):
                            latest_key_name = s3_obj
                            print 'Latest Key Name: %s' % latest_key_name['Key']
