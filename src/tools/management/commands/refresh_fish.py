...
from api.models import (FishFamily, FishGenus, FishGroupFunction, FishGroupTrophic, FishGroupSize, FishSpecies,
                        BaseAttributeModel, APPROVAL_STATUSES)
        ...
        with open(self.source) as fishdata:
            ...
            for row in csvreader:
                ...
                if species_row['group_size'] is not None:
                    group_size, _ = FishGroupSize.objects.get_or_create(name=species_row['group_size'])
                    species_row['group_size'] = group_size
                else:
                    species_row['group_size'] = None

                species_row['max_length_type'] = self.LENGTH_TYPE_DATA_MAP.get(species_row['max_length_type'])
                if species_row['max_length_type'] is None:
                    species_row['max_length_type'] = ''

                species_row['trophic_group'] = self.TROPHIC_GROUP_DATA_MAP.get(species_row['trophic_group'])
                if species_row['trophic_group'] is not None:
                    trophic_group, _ = FishGroupTrophic.objects.get_or_create(name=species_row['trophic_group'])
                ...
                if 'functional_group' in species_row and species_row['functional_group'] != 'NA':
                    species_functionalgroup_name = func_groups[species_row['functional_group'].lower()]
                    functional_group, _ = FishGroupFunction.objects.get_or_create(name=species_functionalgroup_name)
                ...
                fs, _ = FishSpecies.objects.get_or_create(**{'name': species_row['name'], 'genus': genus})
