...
def test_beltfish_su_view(
    ...
    assert data[0]["biomass_kgha_trophic_group"]["other"] == pytest.approx(biomass_kgha_1_other, 0.1)
    assert data[0]["biomass_kgha_trophic_group"]["omnivore"] == pytest.approx(obs_belt_fish1_1_biomass, 0.1)
    ...
    fish_family_biomass_0 = data[0]["biomass_kgha_fish_family"]
...
def test_benthicpit_su_view(
    ...
    assert data[0]["percent_cover_benthic_category"]["Macroalgae"] == 20.0
    assert data[0]["percent_cover_benthic_category"]["Hard coral"] == 60.0
    assert data[0]["percent_cover_benthic_category"]["Rock"] == 20.0

    assert data[1]["percent_cover_benthic_category"]["Macroalgae"] == 60.0
    assert data[1]["percent_cover_benthic_category"]["Hard coral"] == 40.0

    assert data[2]["percent_cover_benthic_category"]["Hard coral"] == 60.0
    assert data[2]["percent_cover_benthic_category"]["Rock"] == 40.0
...
def test_benthiclit_su_view(
    ...
    assert data[0]["percent_cover_benthic_category"]["Macroalgae"] == 16.92
    assert data[0]["percent_cover_benthic_category"]["Hard coral"] == 60.0
    assert data[0]["percent_cover_benthic_category"]["Rock"] == 23.08

    assert data[1]["percent_cover_benthic_category"]["Hard coral"] == 58.46
    assert data[1]["percent_cover_benthic_category"]["Rock"] == 41.54
