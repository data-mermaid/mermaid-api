...
def test_bleaching_colonies_bleached_csv_view(
    ...
    ba_ordered_rownum = 4
    assert len(rows[ba_ordered_rownum].keys()) == 56
...
def test_bleaching_colonies_bleached_field_report(
    ...
    ba_ordered_rownum = 4
    assert len(rows[ba_ordered_rownum].keys()) == 50
