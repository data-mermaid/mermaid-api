...
from api.submission.validations.statuses import ERROR, OK
from api.submission.validations.validators import SampleDateValidator
...
def test_sample_date_validator_too_old_date(valid_collect_record):
    validator = _get_validator()

    record = CollectRecordSerializer(instance=valid_collect_record).data
    record["data"]["sample_event"]["sample_date"] = "1899-12-31"
    result = validator(record)
    assert result.status == ERROR
    assert result.code == SampleDateValidator.IMPLAUSIBLY_OLD_DATE
