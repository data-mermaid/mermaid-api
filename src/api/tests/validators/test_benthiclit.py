...
from api.models import BenthicLIT, BenthicTransect, Observer
...
def existing_benthic_transect(db, sample_event1, profile1):
    ...
    benthiclit = BenthicLIT.objects.create(transect=benthic_transect)
    observer = Observer.objects.create(transectmethod=benthiclit, profile=profile1)

    return observer


def test_benthiclit_validator_ok(valid_benthic_lit_collect_record):
...
def test_benthiclit_validator_data_invalid(valid_benthic_lit_collect_record):
    validator = _get_validator()
    record = CollectRecordSerializer(valid_benthic_lit_collect_record).data
...
def test_benthiclit_validator_duplicate_invalid(
    valid_benthic_lit_collect_record, existing_benthic_transect
):
    validator = _get_validator()
    record = CollectRecordSerializer(valid_benthic_lit_collect_record).data
