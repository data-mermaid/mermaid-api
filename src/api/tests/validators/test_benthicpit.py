...
def _get_validator():
    return UniqueBenthicPITTransectValidator(
...
def test_benthicpit_validator_data_invalid(valid_benthic_pit_collect_record):
    ...
    assert result.code == UniqueBenthicPITTransectValidator.INVALID_DATA
...
def test_benthicpit_validator_duplicate_invalid(
    ...
    assert result.status == ERROR
    assert (
        result.code == UniqueBenthicPITTransectValidator.DUPLICATE_BENTHICPIT_TRANSECT
    )
