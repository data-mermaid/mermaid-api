...
def view_pickle(view_name):
    ...
    dump_cmd_str = 'pg_dump -U {db_user} -h {db_host} -d {db_name} -t public.{view} --schema-only -f {sql_loc}'
    ...
    drop_cmd_str = "psql -U {db_user} -h {db_host} -d {db_name} -c 'DROP MATERIALIZED VIEW public.{view}'"
