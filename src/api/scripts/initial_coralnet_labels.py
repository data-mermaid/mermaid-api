...
def run():
    ...
    # labels_csv = Path(settings.BASE_DIR, "data", "CoralNetMermaidMatchedCoralFocusModel2Reassign_master - labels.csv")
    # initial_classifier = Classifier.objects.get(name="initial pyspacer classifier")
    ...
    #             existing_label = Label.objects.get_or_none(benthic_attribute_id=ba_id, growth_form_id=gf_id)
    #             new_label = Label(id=label_id, benthic_attribute_id=ba_id, growth_form_id=gf_id)
    #             new_label_mappings = []
    #             new_classifier_labels = []
    #             if existing_label:
    #                 label_mappings = LabelMapping.objects.filter(provider="CoralNet", label=existing_label)
