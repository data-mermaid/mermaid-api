...
from api.models.base import Application
from api.utils import get_or_create_safeish
from api.utils.auth0utils import (
    get_user_info,
    get_jwt_token,
    decode,
    is_hs_token,
...
class JWTAuthentication(BaseAuthentication):
    ...
    def _authenticate_profile(self, payload):
        ...
        if '@clients' in sub:
            ...
            msg = ('Invalid claim. sub should contain \'|\' or' +
                   ' \'@clients\': {}').format(sub)
            logger.debug(msg)
            raise exceptions.AuthenticationFailed(msg)
    ...
    def _validate_profile(self, payload):
        ...
        try:
            ...
            profile, is_new = get_or_create_safeish(
            ...
            if is_new is True:
                ...
                if settings.MC_API_KEY is not None and settings.MC_USER is not None and settings.MC_LIST_ID is not None:
                    from mailchimp3 import MailChimp
                    # https://developer.mailchimp.com/documentation/mailchimp/guides/manage-subscribers-with-the
                    # -mailchimp-api/
                    try:
                        client = MailChimp(mc_api=settings.MC_API_KEY,
                                           mc_user=settings.MC_USER)
                        client.lists.members.create(settings.MC_LIST_ID, {
                        ...
                        logger.error("Unable to create mailchimp member {} {} <{}>".format(profile.first_name,
                                                                                           profile.last_name,
                                                                                           profile.email))
