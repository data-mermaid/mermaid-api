...
class Image(BaseModel):
    ...
    photographer = models.CharField(max_length=45, blank=True, null=True)
    water_quality = models.CharField(max_length=45, blank=True, null=True)
    strobes = models.CharField(max_length=200, blank=True, null=True)
    framing = models.CharField(max_length=200, blank=True, null=True)
    ...
    def __str__(self):
        return f"{self.name} - {self.photo_date}"

    @property
    def collect_record(self):
        return CollectRecord.objects.get(id=self.collect_record_id)

    def save(self, *args, **kwargs):
        if self.image and self.name is None:
