...
class BenthicPITObsSQLModel(BaseSUSQLModel):
    ...
    sql = f"""
        WITH benthicpit_obs_cte AS MATERIALIZED (
            WITH se AS (
...
class BenthicPITSUSQLModel(BaseSUSQLModel):
    ...
    sql = f"""
        ...
        life_histories_agg AS (
            ...
            INNER JOIN (
                SELECT pseudosu_id, COUNT(*) AS count
                FROM benthicpit_obs
                GROUP BY pseudosu_id
            ) obs_count
            ON lh.pseudosu_id = obs_count.pseudosu_id
        ...
        ON (benthicpit_su.pseudosu_id = life_histories_agg.pseudosu_id)
